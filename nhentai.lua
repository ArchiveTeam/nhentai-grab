...
find_item = function(url)
  ...
  for pattern, name in pairs({
    ["^https?://nhentai%.net/([^/]+)/([^/]+)/"]="item"
  }) do
    type_, value = string.match(url, pattern)
    if value then
      if type_ == "g" then
        type_ = "gallery"
      end
  ...
  if value and type_ and item_types[type_] then
    return {
      ["value"]=value,
      ["type"]=type_
    }
...
allowed = function(url, parenturl)
  ...
  if string.match(url, "^https?://[^/]*nhentai%.net/login/") then
    return false
  end
  ...
  for pattern, type_ in pairs({
    ...
    local match1, match2 = string.match(url, pattern)
    if match1 then
      if match1 == "g" then
        match1 = "gallery"
      end
      local new_item = match1 .. ":" .. match2
      if item_types[match1] and new_item ~= item_name then
  ...
  if not string.match(url, "^https?://[^/]*nhentai%.net/") then
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url)
    and not string.match(url, "^https?://[ti][0-9]*%.nhentai%.net/") then
    html = read_file(file)
    if item_type == "gallery"
      ...
      local images = json["images"]
      for _, server_num in pairs({"", "2", "3", "5", "7"}) do
        local thumb_extenion = nil
        for _, key in pairs({"thumb", "thumbnail"}) do
          if images[key] then
            thumb_extension = images[key]["t"]
          end
        end
        if not thumb_extension then
          error("Could not find thumbnail extension.")
        end
        check("https://t" .. server_num .. ".nhentai.net/galleries/" .. media_id .. "/cover." .. extension[json["images"]["cover"]["t"]])
        check("https://t" .. server_num .. ".nhentai.net/galleries/" .. media_id .. "/thumb." .. extension[thumb_extension])
        for num, image_data in pairs(json["images"]["pages"]) do
          check("https://i" .. server_num .. ".nhentai.net/galleries/" .. media_id .. "/" .. tostring(num) .. "." .. extension[image_data["t"]])
          check("https://t" .. server_num .. ".nhentai.net/galleries/" .. media_id .. "/" .. tostring(num) .. "t." .. extension[image_data["t"]])
        end
      end
    end
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["nhentai-dyth4rm4e6ys0h1w"] = discovered_items,
    ["urls-0610288ukmxsy47b"] = discovered_outlinks
  }) do
